    ...
    <form action="{{ route('user.cartCheckout') }}" method="post">
        ...
        <div class="row g-4">
            <div class="col-lg-8">
                ...
                <div class="card p-4">
                    <h4 class="fw-bold m-0 mb-3">Metode Pembayaran</h4>
                    @switch(env('IS_PAYMENT'))
                        @case('TRIPAY')
                            <select name="paymentMethod" class="form-control" id="paymentMethod">
                                <option value="">-- Pilih --</option>
                                @foreach ($channels as $key => $channel)
                                <option value="{{ $channel->total_fee->flat.'|'.$channel->code }}" {{ $channel->code == 'QRISC' || $channel->code == 'QRISC2' ? 'selected' : null }}>{{ $channel->name }}</option>
                                @endforeach
                            </select>
                            {{-- @foreach ($channels as $key => $channel)
                                <div class="form-check mb-2">
                                    <input class="form-check-input paymentMethod" type="radio" name="paymentMethod" id="radioDefault{{ $key }}" value="{{ $channel->total_fee->flat.'|'.$channel->code }}">
            ...
            <div class="col-lg-4">
                <div class="card summary-card p-4">
                    ...
                    <button class="btn btn-primary btn-checkout w-100 text-white shadow-lg" id="checkout-btn"
                        type="submit">
...
@section('js')
    ...
    <script>
        ...
        function calculateCart() {
            ...
            // var adminFee = 0;
            if ($('#paymentMethod').val().split('|')[1] == 'QRISC' || $('#paymentMethod').val().split('|')[1] == 'QRISC2') {
                var adminFee = (((subtotal + tax)/100)*0.7)+parseFloat($('#paymentMethod').val().split('|')[0]);
                console.log(adminFee);
            }
            else{
                var adminFee = parseFloat($('#paymentMethod').val().split('|')[0]);
                console.log(adminFee);
            }
            ...
            document.getElementById('summary-fee').innerText = formatIDR(adminFee);
